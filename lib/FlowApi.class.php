...
class FlowApi {
	...
	public function send($service, $params, $method="GET") {
		...
		$response = ($method=="GET") ? $this->httpGet($url, $params) : $this->httpPost($url, $params);		
		if(isset($response["info"])) {
			$code = $response["info"]["http_code"];
			if (!in_array($code, array("200", "400", "401"))) { throw new Exception("Unexpected error occurred. HTTP_CODE: " .$code , $code); }
		}
	...
	private function sign($params) {
		...
		if(!function_exists("hash_hmac")) { throw new Exception("function hash_hmac not exist", 1); }
